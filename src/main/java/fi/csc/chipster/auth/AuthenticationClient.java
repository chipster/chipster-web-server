...
public class AuthenticationClient {
	 ...
	 * @param username
	 * @param password
	 * @param role           Role.CLIENT or Role.SERVER. Only servers validate the
	 *                       token
	 */
	...
	public AuthenticationClient(String authUri, String username, String password, String role,
			boolean startTokenRefreshTimer) {
		this.authenticationServiceUri = authUri;
		this.role = role;
	 ...
	 * 
	 * In the initialization (or after failures) we have to use username and
			...
			if (serverToken != null) {
				...
				if (parsedToken.getValidUntil().isBefore(Instant.now().plus(TOKEN_REFRESH_INTERVAL.multipliedBy(2)))) {
					...
					try {
						...
						logger.info(
								"new token valid until " + parsedToken.getValidUntil() + " (" + this.username + ")");
					} catch (Exception e) {
						logger.warn("getting new token to replace soon expiring token failed (" + this.username + ")",
								e);
					}
				}
	...
	public String createSessionToken(String username, UUID sessionId, Instant valid, Access access)
	...
	public String createDatasetToken(String username, UUID sessionId, UUID datasetId, Instant valid)
			throws RestException {
