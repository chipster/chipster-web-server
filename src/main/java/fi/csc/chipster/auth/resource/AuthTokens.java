 ...
 * 
 * Tokens are signed with auth's key pair. Validation ensures that the token is
 * signed
 * by the auth service, hasn't been tampered with and is still valid.
 * 
 * Otherwise it's up to other services to decide what kind of tokens they allow
 * to
...
public class AuthTokens {
	...
	private static final Duration CLIENT_TOKEN_LIFETIME = Duration.of(3, ChronoUnit.DAYS); // UNIT MUST BE DAYS OR
																							// SHORTER, MONTH IS NOT OK
	private static final Duration CLIENT_TOKEN_MAX_LIFETIME = Duration.of(10, ChronoUnit.DAYS); // UNIT MUST BE DAYS OR
																								// SHORTER, MONTH IS NOT
																								// OK

	private static final Duration SERVER_TOKEN_LIFETIME = Duration.of(6, ChronoUnit.HOURS); // UNIT MUST BE DAYS OR
																							// SHORTER, MONTH IS NOT OK;
	private static final Duration SERVER_TOKEN_MAX_LIFETIME = Duration.of(10, ChronoUnit.DAYS); // UNIT MUST BE DAYS OR
																								// SHORTER, MONTH IS NOT
																								// OK;
	...
	public static String createUserToken(String username, Set<String> roles, Instant loginTime, PrivateKey privateKey,
			SignatureAlgorithm signatureAlgorithm, String name) {
	...
	public String refreshUserToken(String token, String name) {
		...
		if (validToken.getUsername().equals("comp")) {
			logger.info("REFRESH " + validToken.getValidUntil() + " "
					+ getUserTokenNextExpiration(validToken.getCreated(), validToken.getRoles()));
		}
	...
	private static Instant getUserTokenNextCandidateExpiration(Set<String> roles) {
		return roles.contains(Role.SERVER) ? Instant.now().plus(SERVER_TOKEN_LIFETIME)
	...
	private static Instant getUserTokenFinalExpiration(Instant created, Set<String> roles) {
		return roles.contains(Role.SERVER) ? created.plus(SERVER_TOKEN_MAX_LIFETIME)
	...
	public static <T extends ChipsterToken> T validateToken(String jwsString, PublicKey publicKey,
			Class<T> tokenClass) {
	 ...
	 * 
	 * Client can trust the token it got from the server. Servers should always
	 * validate them.
	 * 
	 * @param jwsString
	 * @return
	...
	public static UserToken decodeUserToken(String jwsString) {
		...
		// signature must be removed, jjwt refuses to decode signed tokens without
		// validating them
	...
	public static UserToken claimsToUserToken(Claims body, String jwsString) {
		Set<String> roles = new HashSet<String>(
				Arrays.asList(body.get(CLAIM_KEY_ROLES).toString().split(ROLES_DELIMITER)));
