...
public class RuntimeRepository {
	 ...
	 * @throws Exception
	 */
	public RuntimeRepository(File workDir, InputStream runtimeConfig, Config config) throws CompException {
		this.config = config;
	...
	private synchronized void loadRuntimes(File workDir, InputStream runtimeConfig) throws CompException  { 
		...
		try {
			...
			for (Element runtimeElement: XmlUtil.getChildElements(runtimesElement, "runtime")) {
				...
				JobFactory jobFactory = (JobFactory)Class.forName(handlerClassName).getConstructor(HashMap.class, Config.class).newInstance(parameters, config);
