/* Copy of the CipherInputStream from OpenJDK 21, just buffer size has been increased 512 -> 64k

This improves performance a lot, for example encrypting a file on M1 and SSD went from 140 MiB/s to 700 MiB/s.
There was two problems with the small buffer size. Firstly, it caused too many read request on the input side. 
It would have been easy to combine these small reads by wrapping the inner stream with BufferedInputStream. Secondly,
on the output side, the CipherInputStream always return only 512 bytes, regardless of the buffer size. I couldn't find 
ready-made solution which would continue reading until enough data has been received.
*/
...
public class ChipsterCipherInputStream extends FilterInputStream {
     ...
     */
    private final byte[] ibuffer = new byte[1 << 16];
     ...
     */
    public ChipsterCipherInputStream(InputStream is, Cipher c) {
        super(is);
        input = is;
        cipher = c;
    }
     ...
     * @param is the to-be-processed input stream
     */
    protected ChipsterCipherInputStream(InputStream is) {
        super(is);
        input = is;
