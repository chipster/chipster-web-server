 ...
 * 
 * When service-locator wants to know about services in the request filter, it can't use 
 * the ServiceLocatorClient, because that would make new request, which would again call the filter,
 * creating an infinite loop. This version shares the same interface, but is initialized with static data.
 * 
 * @author klemela
 *
 */
public class LocalServiceLocatorClient extends ServiceLocatorClient {	
	...
	public LocalServiceLocatorClient(ArrayList<Service> publicServices, ArrayList<Service> allServices, Config config) throws IOException {
		super(config);
