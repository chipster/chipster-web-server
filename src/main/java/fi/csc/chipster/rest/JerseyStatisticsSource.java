...
public class JerseyStatisticsSource implements MonitoringStatisticsListener, StatusSource {
	...
	public void onStatistics(MonitoringStatistics statistics) {
		...
		latestStatistics.put("exceptionMappingsSuccessful",
				statistics.getExceptionMapperStatistics().getSuccessfulMappings());
		latestStatistics.put("exceptionMappingsTotal", statistics.getExceptionMapperStatistics().getTotalMappings());
		latestStatistics.put("exceptionMappingsUnsuccessful",
				statistics.getExceptionMapperStatistics().getUnsuccessfulMappings());
		latestStatistics.put("requestAverageDuration",
				statistics.getRequestStatistics().getTimeWindowStatistics().get(15000l).getAverageDuration());
		latestStatistics.put("requestMaximumDuration",
				statistics.getRequestStatistics().getTimeWindowStatistics().get(15000l).getMaximumDuration());
		latestStatistics.put("requestMinimumDuration",
				statistics.getRequestStatistics().getTimeWindowStatistics().get(15000l).getMinimumDuration());
		latestStatistics.put("requestCount",
				statistics.getRequestStatistics().getTimeWindowStatistics().get(15000l).getRequestCount());
		latestStatistics.put("requestsPerSecond",
				statistics.getRequestStatistics().getTimeWindowStatistics().get(15000l).getRequestsPerSecond());
		...
		// statistics.getExceptionMapperStatistics().getExceptionMapperExecutions());
	}
		...
		httpServer.getServerConfiguration().getMonitoringConfig().getWebServerConfig()
				.addProbes(new HttpServerProbe.Adapter() {
					...
					public void onRequestCompleteEvent(HttpServerFilter filter,
							@SuppressWarnings("rawtypes") Connection connection, Response response) {
