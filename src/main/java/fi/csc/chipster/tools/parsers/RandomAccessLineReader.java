...
/**
 * Custom implementation of random access line reading, because method
 * HttpInputStream.skip()
 * doesn't support random access and is therefore useless with big files. File
 * implementation
 * FileInputStream.skip() is workable, but this is used also with files because
 * of consistency and
 * speed. This implementation is fast for getting a few lines anywhere in the
 * file,
 * but sequential performance is poor over http, because requests are made
 * sequentially at the moment.
 * 
 * Jetty server sends only about 4 kilobytes per range request.
 * Sequential throughput varies greatly with ping, from 1 MB/s in local network
 * (1 ms ping)
 * to 10 kB/s (~50 ms ping).
 * 
 * Buffering is needed in http, but with local files it might not be necessary.
 * Probably RandomAccessFile
 * could be queried directly, because file reading is buffered anyway in
 * operating system level.
 * Currently the buffer is used also with files, but the performance penalty
 * doesn't seem to be too bad,
 * because this implementation is still faster than FileInputStream.skip() (but
