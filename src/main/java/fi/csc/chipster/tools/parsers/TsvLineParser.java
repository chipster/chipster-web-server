...
public class TsvLineParser extends AbstractTsvLineParser {
	...
	// public TsvLineParser(DataUrl data, TrackType trackType) throws IOException,
	// URISyntaxException {
	// this(data);
	//
	...
	private TsvLineParser(DataUrl data) throws IOException, URISyntaxException {
		...
		if (splittedHeader.length == splittedContent.length - 1) {
			// Some R results don't have column title for the first column. (similar to
			// generic Chipster implementation in TableColumnProvider)
