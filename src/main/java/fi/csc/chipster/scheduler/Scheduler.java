...
public class Scheduler implements SessionEventListener, MessageHandler.Whole<String>, StatusSource {
	...
	public void startServer()
		...
		this.adminServer = RestUtils.startAdminServer(new AdminResource(this, pubSubServer), null, Role.SCHEDULER,
		...
		synchronized (jobs) {
			...
			case OFFER:
				...
				if (jobs.get(jobIdPair) != null) {
					...
					logger.warn("comp " + asShort(compMsg.getCompId()) + " sent a offer of an non-existing job "
							+ asShort(jobIdPair.getJobId()));
				}
				break;
			case BUSY:
				...
				if (jobs.get(jobIdPair) != null) {
					...
					logger.warn("comp " + asShort(compMsg.getCompId()) + " sent a busy message of an non-existing job "
							+ asShort(jobIdPair.getJobId()));
				}
				break;
			...
			case RUNNING:
				...
				if (jobs.get(jobIdPair) != null) {
					...
					logger.warn("comp " + asShort(compMsg.getCompId()) + " sent a heartbeat of an non-existing job "
							+ asShort(jobIdPair.getJobId()));
				}
	...
	private void handleJobTimer() {
		synchronized (jobs) {
			...
			// expire waiting jobs if any comp haven't accepted it (either because of being
			// full or missing the suitable tool)
			...
			for (IdPair jobIdPair : jobs.getNewJobs().keySet()) {
				...
				if (jobState.isUserLimitReached()) {
					if (jobState.getTimeSinceNew() > waitNewSlotsPerUserTimeout) {
						jobs.remove(jobIdPair);
						expire(jobIdPair,
								"The job couldn't run, because you had the maximum number of jobs running. Please try again after you other jobs have completed.");
					}
				} else if (jobState.isRunnable()) {
					if (jobState.getTimeSinceNew() > waitRunnableTimeout) {
						jobs.remove(jobIdPair);
						expire(jobIdPair,
								"There was no computing server available to run this job, please try again later");
					}
				} else {
	 ...
	 * 
	 * No need to update the db, because saving the SCHEDULE state would only make
	 * it more difficult to resolve the job states when the scheduler is started.
	 * 
	 * @param jobIdPair
	 * 
	 * @param jobIdPair
	 ...
	 * @param jobId
	 */
	private void cancel(IdPair idPair, String stateString) {
		synchronized (jobs) {
	...
	private void scheduleNewJobs() {
		...
		if (newJobs.size() > 0) {
			logger.info("rescheduling " + newJobs.size() + " waiting jobs (" + jobs.getScheduledJobs().size()
