...
public class AuthTokenResourceTest {
	...
	public static void setUp() throws Exception {
		...
		// client with authentication enabled, but each test will set the credentials
		// later
		target = AuthenticationClient.getClient(null, null, true)
				.target(config.getInternalServiceUrls().get(Role.AUTH));
	}
	...
	public void postSessionToken() {
		...
		// SessionTokens must not be allowed to create new tokens (especially directly
		// from auth)
		try {
	...
	public void postDatasetToken() {
		...
		// datasetTokens must not be allowed to create new tokens (especially directly
		// from auth)
		try {
			postToken(target.path(PATH_TOKENS).path(AuthTokenResource.PATH_DATASET_TOKEN),
					"token", datasetToken);
