...
public class EventTest {
	...
	public static void setUp() throws Exception {
		...
		token = new AuthenticationClient(serviceLocator, TestServerLauncher.UNIT_TEST_USER1, "clientPassword",
				Role.CLIENT).getToken().toString();
		token2 = new AuthenticationClient(serviceLocator, TestServerLauncher.UNIT_TEST_USER2, "client2Password",
				Role.CLIENT).getToken().toString();
		schedulerToken = new AuthenticationClient(serviceLocator, Role.SCHEDULER, Role.SCHEDULER, Role.CLIENT)
				.getToken().toString();
	...
	public void authSessionId(boolean retry) throws Exception {
		...
		try {
			getTestClient(uri,
					SessionDbTopicConfig.SESSIONS_TOPIC_PREFIX + launcher.getUser1Credentials().getUsername(), messages,
					latch, retry, token);
			assertEquals(true, false);
		} catch (WebSocketClosedException e) {
			assertEquals(CloseCodes.VIOLATED_POLICY, e.getCloseReason().getCloseCode());
		...
		// unparseable token
		try {
			getTestClient(uri, SessionDbTopicConfig.SESSIONS_TOPIC_PREFIX + sessionId, messages, latch, retry,
					"unparseableToken");
			assertEquals(true, false);
		} catch (WebSocketClosedException e) {
			assertEquals(CloseCodes.VIOLATED_POLICY, e.getCloseReason().getCloseCode());
	...
	public void authUserEvents(boolean retry) throws Exception {
		...
		// unparseable token
		try {
			getTestClient(uri, SessionDbTopicConfig.USERS_TOPIC_PREFIX + userId, messages, latch, retry,
					"unparseableToken");
			assertEquals(true, false);
	...
	public void connectionClose() throws Exception {
		...
		WebSocketClient client = getTestClient(uri, SessionDbTopicConfig.SESSIONS_TOPIC_PREFIX + sessionId, messages,
				latch, false, token);
	...
	public void deleteSession() throws Exception {
		...
		final ArrayList<String> messages = new ArrayList<>();
		final CountDownLatch latch = new CountDownLatch(1);
		WebSocketClient client = getTestClient(SessionDbTopicConfig.SESSIONS_TOPIC_PREFIX + sessionId.toString(),
				messages, latch);
	...
	public void reconnect() throws Exception {
		...
		final ArrayList<String> messages = new ArrayList<>();
		final CountDownLatch latch = new CountDownLatch(1);
		WebSocketClient client = getTestClient(uri, SessionDbTopicConfig.SESSIONS_TOPIC_PREFIX + sessionId.toString(),
				messages, latch, true, token);
	...
	private WebSocketClient getTestClient(String topic, final ArrayList<String> messages, final CountDownLatch latch)
			throws Exception {
		return getTestClient(uri, topic, messages, latch, false, token);
	}

	public static WebSocketClient getTestClient(String requestUri, String topic, final ArrayList<String> messages,
			final CountDownLatch latch, boolean retry, String token) throws Exception {
	...
	public void putJob(String eventToken) throws Exception {
		...
		final CountDownLatch latch = new CountDownLatch(1);
		WebSocketClient client = getTestClient(uri, SessionDbTopicConfig.SESSIONS_TOPIC_PREFIX + sessionId.toString(),
				messages, latch, false, eventToken);
