...
public class TestServerLauncher {
	...
	public Client getUser1Client() {
		return new AuthenticationClient(serviceLocatorClient, UNIT_TEST_USER1, "clientPassword", Role.CLIENT)
				.getAuthenticatedClient();
	}

	public Client getUser2Client() {
		return new AuthenticationClient(serviceLocatorClient, UNIT_TEST_USER2, "client2Password", Role.CLIENT)
				.getAuthenticatedClient();
	}

	public Client getMonitoringClient() {
		return new AuthenticationClient(serviceLocatorClient, Role.MONITORING, Role.MONITORING, Role.CLIENT)
				.getAuthenticatedClient();
	}
	...
	public WebTarget getSchedulerTarget(String role) {
		return new AuthenticationClient(serviceLocatorClient, Role.SCHEDULER, Role.SCHEDULER, Role.SERVER)
				.getAuthenticatedClient().target(getTargetUri(role));
	}

	public WebTarget getCompTarget(String role) {
		return new AuthenticationClient(serviceLocatorClient, Role.COMP, Role.COMP, Role.SERVER)
				.getAuthenticatedClient().target(getTargetUri(role));
	}
	...
	public WebTarget getAuthFailTarget(String role) {
		// password login should be enabled only on auth, but this tries to use it on
		// the session storage
	...
	public CredentialsProvider getSessionWorkerToken() {
		return new AuthenticationClient(serviceLocatorClient, Role.SESSION_WORKER, Role.SESSION_WORKER, Role.SERVER)
				.getCredentials();
	}

	public CredentialsProvider getFileBrokerToken() {
		return new AuthenticationClient(serviceLocatorClient, Role.FILE_BROKER, Role.FILE_BROKER, Role.SERVER)
				.getCredentials();
	}
	 ...
	 * 
	 * Without signature the user could create any kind of token for
	 * himself/herself.
	 * 
	 * @return
	 */
	public CredentialsProvider getUnsignedToken() {
		CredentialsProvider userToken = new AuthenticationClient(serviceLocatorClient, UNIT_TEST_USER1,
				"clientPassword", Role.CLIENT).getCredentials();
	...
	public CredentialsProvider getSignatureFailToken() {
		CredentialsProvider userToken1 = new AuthenticationClient(serviceLocatorClient, UNIT_TEST_USER1,
				"clientPassword", Role.CLIENT).getCredentials();
		CredentialsProvider userToken2 = new AuthenticationClient(serviceLocatorClient, UNIT_TEST_USER2,
				"client2Password", Role.CLIENT).getCredentials();
		String tokenKey1 = userToken1.getPassword();
		String tokenKey2 = userToken2.getPassword();

		String signatureFailToken = getHeader(tokenKey1) + "."
	 ...
	 * The algorithm is send in the JWT header
	 * 
	 * Make sure the server doesn't accept tokens that are signed with the public
	 * key (as a symmetric key)
	 * https://auth0.com/blog/critical-vulnerabilities-in-json-web-token-libraries/
	 * 
	 * @return
	 * @throws IOException
	 ...
	 */
	public CredentialsProvider getSymmetricToken()
			throws IOException, InvalidKeySpecException, NoSuchAlgorithmException {

		byte[] publicKey = new AuthenticationClient(new ServiceLocatorClient(config), "session-db", "session-db",
				Role.SERVER).getJwtPublicKey().getEncoded();

		/*
		 * https://stackoverflow.com/a/68377829
		 * 
		 * "SecretKey[Spec] classes and their subclasses are used only for symmetric
		 * algorithms,
		 * including HMAC, and the PrivateKey[Spec] classes and their subclasses are
		 * used only
		 * for asymmetric algorithms"
		 */
		SecretKeySpec symmetricKey = new SecretKeySpec(publicKey, HmacAlgorithms.HMAC_SHA_512.getName());

		// have to sign a token here, because AuthTokens.createUserToken() doesn't
		// (correctly)
		// accept a symmetric key

		HashSet<String> roles = new HashSet<String>(
				Arrays.asList(new String[] { Role.CLIENT, Role.SERVER, Role.ADMIN }));
